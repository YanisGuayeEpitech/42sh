...
Test(my_hash_map_get, not_present_empty_map)
{
    my_hash_map_t map;
    ...
    cr_assert_eq(NULL, my_hash_map_get(&map, &key));
    cr_assert_eq(MY_MAP_OK, my_hash_map_init_capacity(&map, &kvtypes, 16));
    cr_assert_eq(NULL, my_hash_map_get(&map, &key));
    my_hash_map_drop(&map);
}

Test(my_hash_map_get, not_present_map_with_one_item)
{
    my_hash_map_t map;
    my_map_kvtypes_t kvtypes = {
        .key_size = sizeof(char *),
