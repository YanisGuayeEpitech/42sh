...
static int place_prefix(my_iostream_t *output, p_my_printf_conv_t const *conv)
{
    ...
    case P_MY_PRINTF_SPEC_LOWER_BINARY:
        return my_fputs("0b", output);
    case P_MY_PRINTF_SPEC_UPPER_BINARY:
        return my_fputs("0B", output);
    case P_MY_PRINTF_SPEC_OCTAL:
        return my_fputc('0', output);
    case P_MY_PRINTF_SPEC_LOWER_HEXADECIMAL:
        return my_fputs("0x", output);
...
static int print_uint(my_iostream_t *output,
p_my_printf_conv_t const *conv, size_t len, size_t zeros)
{
    ...
    p_my_printf_zpad(output, zeros) ||
    place_prefix(output, conv) ||
