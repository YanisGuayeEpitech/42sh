...
static uintmax_t p_my_printf_get_uint_arg(p_my_printf_arg_type_t type,
va_list va_args)
{
    switch (type) {
    ...
    case P_MY_PRINTF_ARG_UINTMAX_T:
        return va_arg(va_args, intmax_t);
    case P_MY_PRINTF_ARG_SIZE_T:
        return (uintmax_t)va_arg(va_args, size_t);
    case P_MY_PRINTF_ARG_UPTRDIFF_T:
        return (uintmax_t)va_arg(va_args, ptrdiff_t);
    default:
        return (uintmax_t)va_arg(va_args, unsigned int);
    }
}

static uintmax_t p_my_printf_get_sint_arg(p_my_printf_arg_type_t type,
va_list va_args)
{
    switch (type) {
    ...
    case P_MY_PRINTF_ARG_LONG:
        return (intmax_t)va_arg(va_args, long);
    case P_MY_PRINTF_ARG_LONG_LONG:
        return (intmax_t)va_arg(va_args, long long);
    case P_MY_PRINTF_ARG_INTMAX_T:
        return va_arg(va_args, intmax_t);
    ...
    default:
        return (intmax_t)va_arg(va_args, int);
    }
}

static void p_my_printf_fill_int_arg(p_my_printf_arg_t *arg,
...
static void p_my_printf_fill_arg(p_my_printf_arg_t *arg,
p_my_printf_arg_type_t *type, va_list va_args)
{
    switch (*type) {
    ...
    case P_MY_PRINTF_ARG_LONG_DOUBLE:
        ...
        break;
    case P_MY_PRINTF_ARG_PERCENT:
        break;
    default:
        p_my_printf_fill_int_arg(arg, type, va_args);
    }
}
...
p_my_printf_arg_type_t types[], va_list va_args)
{
    for (size_t i = 0; i < count; ++i) {
        ...
        case P_MY_PRINTF_ARG_PTR_PTRDIFF_T:
            ...
            break;
        default:
            p_my_printf_fill_arg(args + i, types + i, va_args);
        }
    }
}
