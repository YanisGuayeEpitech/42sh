...
static void store_char_count2(p_my_printf_conv_t const *conv, size_t char_count)
{
    switch (conv->type) {
    ...
    case P_MY_PRINTF_ARG_PTR_PTRDIFF_T:
        ...
        break;
    default:
        (*conv->value.int_ptr_arg) = (int)char_count;
    }
}

static void store_char_count1(p_my_printf_conv_t const *conv, size_t char_count)
{
    switch (conv->type) {
    ...
    case P_MY_PRINTF_ARG_PTR_INTMAX_T:
        ...
        break;
    default:
        store_char_count2(conv, char_count);
    }
}

MY_LOCAL int p_my_printf_conversion(my_iostream_t *output,
